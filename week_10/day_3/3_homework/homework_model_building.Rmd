...
house_data_tidy <- house_data %>%
  ...
  fastDummies::dummy_cols(select_columns = "grade", remove_first_dummy = TRUE, remove_selected_columns = TRUE) %>%
  mutate(yr_renovated = ifelse(yr_renovated == 0, 0, 1))
...
```{r}
house_data_trim <- house_data_tidy %>% 
  select(-c(sqft_above, sqft_basement, 15:29))

alias(lm(price ~ ., data = house_data_trim))
```
...
```{r, eval=FALSE}
houses_tidy_numeric <- house_data_trim %>%
  select_if(is.numeric)

houses_tidy_nonnumeric <- house_data_trim%>%
  select_if(function(x) !is.numeric(x))

houses_tidy_nonnumeric$price <- house_data_trim$price
...
```{r}
model_1a <- lm(price ~ sqft_living, data = houses_tidy_numeric)
summary(model_1a)
```
...
```
```{r}
model_2 <- lm(price ~ sqft_living + view, data = houses_tidy_numeric)
summary(model_2)
```

```{r}
model_2a <- lm(price ~ sqft_living + lat, data = houses_tidy_numeric)
summary(model_2a)
```
...
```
```{r}
model_3a <- lm(price ~ sqft_living + lat + view, data = houses_tidy_numeric)
summary(model_3a)
```


```{r}
model_3b <- lm(price ~ sqft_living + lat + yr_renovated, data = houses_tidy_numeric)
summary(model_3b)
```
...
```{r}
model_4a <- lm(price ~ sqft_living + lat +  view + yr_built, data = houses_tidy_numeric)
summary(model_4a)
```
