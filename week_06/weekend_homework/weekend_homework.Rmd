...
*  $H_0$ and $H_a$ in **both** mathematical notation and in words. 
    H_0: π_aware = 0.4
        The proportion of the sample that are aware of the coffee shop os equal to 40%
    H_A: π_aware < 0.4
        The alternative hypothesis is that the proportion of sample aware of the coffee shop is less than 40%
* Also state the method you would use to generate the null distribution (bootstrap, permutation or simulation). 
...
*  $H_0$ and $H_a$ in **both** mathematical notation and in words. 
    H_0: µ_right -  µ_top = 0
        The difference of the means of click through between sample where banner on right of page compared the banner on the top is zero. Therefore no difference if the banner is on top or bottom.
    H_A: µ_right -  µ_top > 0
        The alternative hypothesis is that the mean of click throughs when banner on the right is greater than that when the banner is on the top
* Also state the method you would use to generate the null distribution (bootstrap, permutation or simulation). 
    You would use the permutation
...
* What kind of test you would use  
  One-sample mean
*  $H_0$ and $H_a$ in **both** mathematical notation and in words.145 
    H_0: µ_width = 145
        The width of the parts is equal to 145mm
    H_A: µ_width != 145
        The alternative hypothesis is that the width of the parts is statistically significantly different from 
* Also state the method you would use to generate the null distribution (bootstrap, permutation or simulation). 
    You would use the bootstrap
...
We lack sufficient evidence to reject H0 and so we fail to reject H0. Based on our sample, we do not have enough evidence that the proportion is statistically significantly differently greater than 40%.
...
p-value is less than α we reject H0 in favour of Ha. We found enough evidence in the sample to suggest that the sample mean is statistically significantly different from than the null value and it is statistically likely the website will get more clicks if the banner is on the right.
<br>
...
We lack sufficient evidence to reject H0 and so we fail to reject H0. Based on our sample, we do not have enough evidence that the proportion is statistically significantly differently greater than 145mm. Therefore it is significantly likely that width of the part is close to 145mm
