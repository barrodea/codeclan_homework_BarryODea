...
```{r}
ci_99 <- bootstrap_sample_200 %>%
  summarise(
    mean = mean(mean_lot_area),
    lower_bound = quantile(mean_lot_area, probs = 0.01),
    upper_bound = quantile(mean_lot_area, probs = 0.99)
  )
...
```{r}
point_estimate <- bootstrap_sample_200 %>% 
