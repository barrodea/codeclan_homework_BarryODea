...
titanic <- titanic %>% 
  mutate(crew = case_when(
     ...
     class %in% c("engineering crew", "victualling crew", "restaurant staff", "deck crew") ~ TRUE
  ))

titanic %>% count(crew)
```

# Probability of crew surviving
```{r}
prob_crew_survive <- titanic %>%
  filter(crew == TRUE) %>%
...
# Probability of crew surviving
```{r}
prob_passanger_survive <- titanic %>%
    filter(crew == FALSE) %>%
  summarise(prob = sum(survived == "yes") / n())
prob_passanger_survive
```
...
s_sum_equals_5_6_7 <- s_four_dice %>% 
  filter(total == 5 | total == 6 | total == 7)

# how many outcomes are in the event where the total of the four dice equals $5$, $6$ or $7$)?
nrow(s_sum_equals_5_6_7)

#what is the probability of this event?
sum_prob <- s_four_dice %>% 
 filter(total == 5 | total == 6 | total == 7) %>% 
