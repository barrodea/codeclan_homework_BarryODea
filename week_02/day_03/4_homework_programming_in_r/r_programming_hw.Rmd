...
#create lists for the next 3 days waether forecast
weather_forcast <- list(
  "Thursday" = list(temperature = 17, rain_chance = 10, wind_speed = 5), 
  "Friday" = list(temperature = 13, rain_chance = 60, wind_speed = 15),
  "Saturday" = list(temperature = 14, rain_chance = 40, wind_speed = 17) 
)
...
```{r}
# pull out chance of rain in tomorrow's list
weather_forcast$Thursday$rain_chance
```
  
  
  ii. The wind speed on the day after tomorrow
```{r}
# pull out wind speed from Friday's list
weather_forcast$Friday$wind_speed
```
  
  
  iii. The temperature tomorrow.
```{r}
# pull out temperature from Thursday's list
weather_forcast$Thursday$temperature
```
...
```{r}
# create 7 day forecast and create matrix

temperatures <- c(12, 14, 17, 13, 18, 10, 11, 15, 16, 12, 19, 9, 11, 10)
weekly_temperatures <- matrix(temperatures, nrow = 2, ncol = 7)
weekly_temperatures
```
...
```{r}
# extract the 3rd column in the 2nd row
weekly_temperatures[2, 3]
```
  
  ii. The whole second week

```{r}
# extract all of the 2nd row
weekly_temperatures[2, ]
```
  
  iii. The first, third and fifth days in both weeks.
```{r}
# extract the 1st, 3rd and fifth columns of both rows
weekly_temperatures[c(1, 2), c(1, 3, 5)]
```
...
# create function for x <, >, = 10

over_or_under_10 <- function(x){
if (x < 10){ 
...
```{r}
compare_2_vectors <- function(x, y)
  if (identical(x, y)){
