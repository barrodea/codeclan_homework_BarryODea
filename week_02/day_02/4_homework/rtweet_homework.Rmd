...
```{r}
# total of favorities
# filter out where it is a quote and sum the favorite count
code_clan_tweets %>% 
...
```{r}
# filter out where it is a quote, group by source and calculate mean of retweet_count
code_clan_tweets %>% 
  filter(is_quote == FALSE) %>% 
  group_by(source) %>% 
...
```{r}
´#replace NA in media_type with "text" and then sum the favorite_count by medai type
...
```{r}
# get mean using display_text_width
...
# left join two datasets by "tweet_id"
tweets_with_url <- 
  left_join(code_clan_tweets, code_clan_info, "tweet_id")
