...
```{r}
# detect if 'Thought Police appears checking for upper and lowercase
whole_text %>% 
...
```{r}
# use str_count for Big Brother using upper and lowercase
whole_text %>% 
...
```{r}
# detect if 'Julia' appears checking for upper and lowercase
whole_text %>% 
...
```{r}
#create pattern for date and extract from data

pattern_date <- "[A-Z][a-z]+\\s[0-9][a-z]+,\\s[0-9]+"

whole_text %>% 
  str_extract(pattern_date)
```
...
```{r}
# use str_replace_all to redact both Winstom and Smith

author_redacted <- whole_text %>% 
  str_replace_all("Winston", "REDACTED") %>% 
  str_replace_all("Smith", "REDACTED")
...
# find any matches for Victory Gin or gin and replace with vodka using str_replace
author_redacted %>% 
  str_extract_all("[Vv][Ii][Cc][Tt][Oo][Rr][Yy] [Gg][Ii][Nn]")

author_redacted %>% 
  str_extract_all("[Gg][Ii][Nn]")
  
gin_to_vodka <- author_redacted %>%
  str_replace_all("VICTORY GIN", "VODKA") %>% 
...
```{r}
# check for "department" and replace with "section"
gin_to_vodka %>% 
  str_extract_all("[Dd]epartment")
gin_to_vodka %>% 
  str_extract_all("[Dd]ep")

section_change <- whole_text %>% 
  str_replace_all("Department", "Section") %>% 
  str_replace_all("Dep", "Sec")

```

```{r}
# check for "ministry" and replace with "department"
section_change %>% 
  str_extract_all("[Mm]inistry")
whole_text %>% 
  str_extract_all("[Mm]ini")

department_change <- section_change %>% 
